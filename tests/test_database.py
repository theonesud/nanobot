...
class TestDatabase:
    ...
    def db(self, temp_workspace):
        """Create a database instance with temp workspace."""
    ...
    def test_task_costs_table_exists(self, db):
        """Test task_costs table is created."""
    ...
    def test_active_crons_table_exists(self, db):
        """Test active_crons table is created."""
        with sqlite3.connect(db.db_path) as conn:
            cursor = conn.execute(
    ...
    def test_log_cost(self, db):
        ...
        with sqlite3.connect(db.db_path) as conn:
            ...
            assert row[4] == 1000  # prompt_tokens
            assert row[5] == 500  # completion_tokens
            assert row[6] == 0.03  # cost_usd

    def test_log_cost_defaults(self, db):
        """Test logging cost with default values."""
        db.log_cost(
    ...
    def test_get_daily_cost_empty(self, db):
        """Test getting daily cost when no costs logged."""
        cost = db.get_daily_cost()
        assert cost == 0.0

    def test_multiple_sessions(self, db):
        """Test logging costs for multiple sessions."""
