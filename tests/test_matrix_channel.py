...
from nanobot.channels.matrix import MATRIX_HTML_FORMAT, TYPING_NOTICE_TIMEOUT_MS, MatrixChannel
...
async def test_start_skips_load_store_when_device_id_missing(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        "nanobot.channels.matrix.AsyncClientConfig", lambda **kwargs: SimpleNamespace(**kwargs)
    )
...
async def test_on_message_allowlist_policy_requires_room_id() -> None:
    ...
    allowed_room = SimpleNamespace(
        room_id="!allowed:matrix.org", display_name="Allowed", member_count=3
    )
...
async def test_on_media_message_decrypts_encrypted_media(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr("nanobot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        matrix_module, "decrypt_attachment", lambda ciphertext, key, sha256, iv: b"plain"
    )
    channel = MatrixChannel(_make_config(), MessageBus())
    client = _FakeAsyncClient("", "", "", None)
...
async def test_send_adds_thread_relates_to_for_thread_metadata() -> None:
    ...
    client = _FakeAsyncClient("", "", "", None)
    channel.client = client
    metadata = {"thread_root_event_id": "$root1", "thread_reply_to_event_id": "$reply1"}
    await channel.send(
        OutboundMessage(
...
async def test_send_uses_encrypted_media_payload_in_encrypted_room(tmp_path) -> None:
    ...
    await channel.send(
        OutboundMessage(
            channel="matrix", chat_id="!encrypted:matrix.org", content="", media=[str(file_path)]
        )
    )
    assert len(client.upload_calls) == 1
...
async def test_send_does_not_parse_attachment_marker_without_media(tmp_path) -> None:
    ...
    await channel.send(
        OutboundMessage(
            channel="matrix", chat_id="!room:matrix.org", content=f"[attachment: {missing_path}]"
        )
    )
...
async def test_send_passes_thread_relates_to_to_attachment_upload(monkeypatch) -> None:
    ...
    async def _fake_upload_and_send_attachment(
        room_id: str, path: Path, limit_bytes: int, relates_to: dict[str, object] | None = None
    ) -> str | None:
...
async def test_send_workspace_restriction_blocks_external_attachment(tmp_path) -> None:
    ...
    channel = MatrixChannel(
        _make_config(), MessageBus(), restrict_to_workspace=True, workspace=workspace
    )
    client = _FakeAsyncClient("", "", "", None)
    channel.client = client
    await channel.send(
        OutboundMessage(
            channel="matrix", chat_id="!room:matrix.org", content="", media=[str(file_path)]
        )
    )
    assert client.upload_calls == []
    assert len(client.room_send_calls) == 1
