...
app = typer.Typer(
    name="nanobot", help=f"{__logo__} nanobot - Personal AI Assistant", no_args_is_help=True
)
console = Console()
...
def _init_prompt_session() -> None:
    ...
    _PROMPT_SESSION = PromptSession(
        history=FileHistory(str(history_file)), enable_open_in_editor=False, multiline=False
    )
...
class GatewayServer:
    ...
    async def run(self):
        try:
            ...
            await asyncio.gather(self.agent.run(), self.channels.start_all())
...
class InteractiveAgentRunner:
    ...
    async def run_interactive(self, session_id: str):
        ...
        signal.signal(
            signal.SIGINT, lambda s, f: (_restore_terminal(), self.agent_loop.stop(), sys.exit(0))
        )
...
def cron_remove(job_id: str = typer.Argument(..., help="Job ID to remove")):
    store_path = get_data_dir() / "cron" / "jobs.json"
    service = CronService(store_path)
    ...
    try:
        ...
        if not (token and token.access):
            ...
            token = login_oauth_interactive(
                print_fn=lambda s: console.print(s), prompt_fn=lambda s: typer.prompt(s)
            )
        if not (token and token.access):
